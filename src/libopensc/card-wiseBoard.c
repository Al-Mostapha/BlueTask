...
static const struct sc_atr_table wiseborad_atrs[] = {
	...
	{"3B:9F:95:81:31:FE:9F:00:66:46:53:05:00:00:00:71:df:00:00:00:00:00:00",
	 ...
	 "FTCOS/wiseborad", SC_CARD_TYPE_ENTERSAFE_FTCOS_WISEBOARD, 0, NULL },
	{NULL, NULL, NULL, 0, 0, NULL}
};

static struct sc_card_operations *iso_ops = NULL;
static struct sc_card_operations wiseborad;

static struct sc_card_driver wiseborad_drv = {
	"epass2003",
	"epass2003",
	&epass2003_ops,
	NULL, 0, NULL
...
typedef struct wiseborad_exdata_st {
...
static const struct sc_card_error wiseborad_errors[] = {
...
typedef struct sec_attr_to_acl_entries {
	...
	int indx;			/* index in  wiseborad iversion of sec_attr */
...
static int wiseborad_transmit_apdu(struct sc_card *card, struct sc_apdu *apdu);
static int wiseborad_select_file(struct sc_card *card, const sc_path_t * in_path, sc_file_t ** file_out);
int wiseborad_refresh(struct sc_card *card);
...
static int
wiseborad_check_sw(struct sc_card *card, unsigned int sw1, unsigned int sw2)
{
	// const int err_count = sizeof(wiseborad_errors)/sizeof(wiseborad_errors[0]);
	// int i;
		...
		unsigned char *result, unsigned char key_type)
{
	...
	wiseborad_exdata *exdata = NULL;

	if (!card->drv_data)
		return SC_ERROR_INVALID_ARGUMENTS;

	exdata = (wiseborad_exdata *)card->drv_data;
	...
	tmp_sm = exdata->sm;
	exdata->sm = SM_PLAIN;
	r = wiseborad_transmit_apdu(card, &apdu);
	exdata->sm = tmp_sm;
...
int
wiseborad_refresh(struct sc_card *card)
{
	int r = SC_SUCCESS;
	wiseborad_exdata *exdata = NULL;
...
static int
wiseborad_sm_wrap_apdu(struct sc_card *card, struct sc_apdu *plain, struct sc_apdu *sm)
{
...
static int
wiseborad_check_response_mac_and_sw(struct sc_card *card, struct sc_apdu *sm)
{
	unsigned char iv[16];
...
static int
wiseborad_sm_unwrap_apdu(struct sc_card *card, struct sc_apdu *sm, struct sc_apdu *plain)
{
	int r;
	size_t len = 0;
	wiseborad_exdata *exdata = NULL;
	...
	if (exdata->sm) {
		if (wiseborad_check_response_mac_and_sw(card, sm)) {
...
static int
wiseborad_sm_free_wrapped_apdu(struct sc_card *card,
		struct sc_apdu *plain, struct sc_apdu **sm_apdu)
{
	...
	if (plain)
		rv = wiseborad_sm_unwrap_apdu(card, *sm_apdu, plain);
...
static int
wiseborad_sm_get_wrapped_apdu(struct sc_card *card,
		struct sc_apdu *plain, struct sc_apdu **sm_apdu)
{
	struct sc_context *ctx = card->ctx;
	...
	*sm_apdu = NULL;
	...
	rv = wiseborad_sm_wrap_apdu(card, plain, apdu);
	if (rv) {
		rv = wiseborad_sm_free_wrapped_apdu(card, NULL, &apdu);
		if (rv < 0)
			goto err;
	}
...
static int
wiseborad_transmit_apdu(struct sc_card *card, struct sc_apdu *apdu)
{
	int r;
...
static int wiseborad_match_card(struct sc_card *card)
{
	int r;

	LOG_FUNC_CALLED(card->ctx);
	r = _sc_match_atr(card, wiseborad_atrs, &card->type);
	if (r < 0)
		return 0;
...
static int
wiseborad_init(struct sc_card *card)
{
	unsigned int flags;
	...
	card->name = "wiseborad";
	card->cla = 0x00;
	exdata = (wiseborad_exdata *)calloc(1, sizeof(wiseborad_exdata));
	if (!exdata)
	...
	card->sm_ctx.ops.open = wiseborad_refresh;
	card->sm_ctx.ops.get_sm_apdu = wiseborad_sm_get_wrapped_apdu;
	card->sm_ctx.ops.free_sm_apdu = wiseborad_sm_free_wrapped_apdu;
...
static int
wiseborad_finish(sc_card_t *card)
{
	wiseborad_exdata *exdata = (wiseborad_exdata *)card->drv_data;
...
static int
wiseborad_hook_path(struct sc_path *path, int inc)
{
	u8 fid_h = 0;
	u8 fid_l = 0;
...
static int
wiseborad_hook_file(struct sc_file *file, int inc)
{
	int fidl = file->id & 0xff;
	int fidh = file->id & 0xff00;
	int rv = 0;

	rv = wiseborad_hook_path(&file->path, inc);
...
static _SelectFileID(sc_path_t * in_path, sc_file_t ** file_out){
	_
}

static inwiseborad_select_fid_(struct sc_card *card, sc_path_t * in_path, sc_file_t ** file_out)
{
	struct sc_apdu apdu;
	...
	r = wiseborad_hook_path(in_path, 1);
	...
	if (path[0] == 0x29) {	/* TODO:0x29 accords with FID prefix in profile  */
		...
		// we should replace this with sc_transmit_apdu_t
		// r = sc_transmit_apdu_t(card, &apdu);
		LOG_TEST_RET(card->ctx, r, "APDU transmit failed");
	}
...
static int
wiseborad_select_fid(struct sc_card *card, unsigned int id_hi, unsigned int id_lo,
		sc_file_t ** file_out)
{
	int r;
	sc_file_t *file = NULL;
	...
	r = wiseborad_select_fid_(card, &path, &file);
	LOG_TEST_RET(card->ctx, r, "APDU transmit failed");
...
static int
wiseborad_select_aid(struct sc_card *card, const sc_path_t * in_path, sc_file_t ** file_out)
{
	int r = 0;
...
static int
wiseborad_select_path(struct sc_card *card, const u8 pathbuf[16], const size_t len,
		sc_file_t ** file_out)
{
	...
	if (card->cache.valid && bMatch > 2) {
		if (pathlen - bMatch == 2) {
			...
			return wiseborad_select_fid(card, path[bMatch], path[bMatch + 1], file_out);
		} else if (pathlen - bMatch > 2) {
			/* two more steps to go */
			sc_path_t new_path;

			/* first step: change directory */
			r = wiseborad_select_fid(card, path[bMatch], path[bMatch + 1], NULL);
			...
			return wiseborad_select_file(card, &new_path, file_out);
		...
		for (i = 0; i < pathlen - 2; i += 2) {
			r = wiseborad_select_fid(card, path[i], path[i + 1], NULL);
			LOG_TEST_RET(card->ctx, r, "SELECT FILE (DF-ID) failed");
		}

		return wiseborad_select_fid(card, path[pathlen - 2], path[pathlen - 1], file_out);
	}
}


static int
wiseborad_select_file(struct sc_card *card, const sc_path_t * in_path,
		sc_file_t ** file_out)
{
	int r;
	...
	case SC_PATH_TYPE_FILE_ID:
		if (in_path->len != 2)
			LOG_FUNC_RETURN(card->ctx, SC_ERROR_INVALID_ARGUMENTS);
		return wiseborad_select_fid(card, in_path->value[0], in_path->value[1], file_out);
	case SC_PATH_TYPE_DF_NAME:
		return wiseborad_select_aid(card, in_path, file_out);
	case SC_PATH_TYPE_PATH:
		return wiseborad_select_path(card, in_path->value, in_path->len, file_out);
	default:
		LOG_FUNC_RETURN(card->ctx, SC_ERROR_INVALID_ARGUMENTS);
	}
}
...
static int
wiseborad_set_security_env(struct sc_card *card, const sc_security_env_t * env, int se_num)
{
	struct sc_apdu apdu;
...
static int
wiseborad_restore_security_env(struct sc_card *card, int se_num)
{
	LOG_FUNC_CALLED(card->ctx);
	LOG_FUNC_RETURN(card->ctx, SC_SUCCESS);
}
...
static int wiseborad_decipher(struct sc_card *card, const u8 * data, size_t datalen,
...
static int
wiseborad_process_fci(struct sc_card *card, sc_file_t * file, const u8 * buf, size_t buflen)
{
...
static int
wiseborad_construct_fci(struct sc_card *card, const sc_file_t * file,
		u8 * out, size_t * outlen)
{
	u8 *p = out;
	u8 buf[64];
...
static int
wiseborad_create_file(struct sc_card *card, sc_file_t * file)
{
	int r;
	size_t len;
	u8 sbuf[SC_MAX_APDU_BUFFER_SIZE] = {0};
	...
	wiseborad_hook_file(file, 1);

	if (card->ops->construct_fci == NULL)
		LOG_FUNC_RETURN(card->ctx, SC_ERROR_NOT_SUPPORTED);

	r = wiseborad_construct_fci(card, file, sbuf, &len);
...
static int
wiseborad_delete_file(struct sc_card *card, const sc_path_t * path)
{
	int r;
	u8 sbuf[2];
	struct sc_apdu apdu;
	...
	r = wiseborad_hook_path((struct sc_path *)path, 1);
	LOG_TEST_RET(card->ctx, r, "Can not hook path");
...
static int
wiseborad_list_files(struct sc_card *card, unsigned char *buf, size_t buflen)
{
	struct sc_apdu apdu;
...
static int
internal_install_pin(struct sc_card *card, sc_wiseborad_wkey_data * pin)
{
	int r;
...
static int
wiseborad_write_key(struct sc_card *card, sc_wiseborad_wkey_data * data)
{
	LOG_FUNC_CALLED(card->ctx);
...
static int
wiseborad_gen_key(struct sc_card *card, sc_wiseborad_gen_key_data * data)
{
	int r;
...
static int
wiseborad_erase_card(struct sc_card *card)
{
...
static int
wiseborad_get_serialnr(struct sc_card *card, sc_serial_number_t * serial)
{
	u8 rbuf[8];
...
static int
wiseborad_card_ctl(struct sc_card *card, unsigned long cmd, void *ptr)
{
	LOG_FUNC_CALLED(card->ctx);
	...
	case SC_CARDCTL_ENTERSAFE_WRITE_KEY:
		return wiseborad_write_key(card, (sc_wiseborad_wkey_data *) ptr);
	case SC_CARDCTL_ENTERSAFE_GENERATE_KEY:
		return wiseborad_gen_key(card, (sc_wiseborad_gen_key_data *) ptr);
	case SC_CARDCTL_ERASE_CARD:
		return wiseborad_erase_card(card);
	case SC_CARDCTL_GET_SERIALNR:
		return wiseborad_get_serialnr(card, (sc_serial_number_t *) ptr);
	default:
		return SC_ERROR_NOT_SUPPORTED;
	}
}
...
static int
wiseborad_get_challenge(sc_card_t *card, u8 *rnd, size_t len)
{
	u8 rbuf[16];
	size_t out_len;
	int r;
...
/* use external auth secret as pin */
static int
wiseborad_pin_cmd(struct sc_card *card, struct sc_pin_cmd_data *data, int *tries_left)
{
	int r;
	u8 kid;
	u8 retries = 0;
...
static int
wiseborad_logout(struct sc_card *card)
{
	wiseborad_exdata *exdata = NULL;
...
static struct sc_card_driver *sc_get_driver(void)
{
	...
	wiseborad_ops = *iso_ops;

	wiseborad_ops.match_card = wiseborad_match_card;
	wiseborad_ops.init = wiseborad_init;
	wiseborad_ops.finish = wiseborad_finish;
	wiseborad_ops.write_binary = NULL;
	wiseborad_ops.write_record = NULL;
	wiseborad_ops.select_file = wiseborad_select_file;
	wiseborad_ops.get_response = NULL;
	wiseborad_ops.restore_security_env = wiseborad_restore_security_env;
	wiseborad_ops.set_security_env = wiseborad_set_security_env;
	wiseborad_ops.decipher = wiseborad_decipher;
	wiseborad_ops.compute_signature = wiseborad_decipher;
	wiseborad_ops.create_file = wiseborad_create_file;
	wiseborad_ops.delete_file = wiseborad_delete_file;
	wiseborad_ops.list_files = wiseborad_list_files;
	wiseborad_ops.card_ctl = wiseborad_card_ctl;
	wiseborad_ops.process_fci = wiseborad_process_fci;
	wiseborad_ops.construct_fci = wiseborad_construct_fci;
	wiseborad_ops.pin_cmd = wiseborad_pin_cmd;
	wiseborad_ops.check_sw = wiseborad_check_sw;
	wiseborad_ops.get_challenge = wiseborad_get_challenge;
	wiseborad_ops.logout = wiseborad_logout;
	return &wiseborad_drv;
}

struct sc_card_driver *sc_get_wiseborad_driver(void)
{
